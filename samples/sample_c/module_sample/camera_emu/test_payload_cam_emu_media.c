...
static void *UserCameraMedia_SendVideoTask(void *arg)
{
    ...
    while (1) {
        ...
        send:
            if (fpFile == NULL) {
                USER_LOG_INFO("auto-start video stream with default file");
                snprintf(videoFilePath, DJI_FILE_PATH_SIZE_MAX, "%s", tempPath);
                startTimeMs = 0;
                sendVideoFlag = true;
                sendOneTimeFlag = false;

                T_DjiReturnCode rc = DjiPlayback_VideoFileTranscode(videoFilePath, "h264", transcodedFilePath, DJI_FILE_PATH_SIZE_MAX);
                if (rc != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                    USER_LOG_ERROR("transcode video file error: 0x%08llX.", rc);
                    osalHandler->TaskSleepMs(1000);
                    continue;
                }
                rc = DjiPlayback_GetFrameRateOfVideoFile(transcodedFilePath, &frameRate);
                if (rc != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                    USER_LOG_ERROR("get frame rate error: 0x%08llX.", rc);
                    osalHandler->TaskSleepMs(1000);
                    continue;
                }
                rc = DjiPlayback_GetFrameInfoOfVideoFile(transcodedFilePath, frameInfo, VIDEO_FRAME_MAX_COUNT, &frameCount);
                if (rc != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                    USER_LOG_ERROR("get frame info error: 0x%08llX.", rc);
                    osalHandler->TaskSleepMs(1000);
                    continue;
                }
                rc = DjiPlayback_GetFrameNumberByTime(frameInfo, frameCount, &frameNumber, startTimeMs);
                if (rc != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                    USER_LOG_ERROR("get frame number error: 0x%08llX.", rc);
                    osalHandler->TaskSleepMs(1000);
                    continue;
                }
                fpFile = fopen(transcodedFilePath, "rb+");
