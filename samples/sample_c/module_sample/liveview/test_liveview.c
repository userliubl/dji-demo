...
T_DjiReturnCode DjiTest_LiveviewRunSample(E_DjiMountPosition mountPosition)
{
    ...
    if (mountPosition >= DJI_MOUNT_POSITION_PAYLOAD_PORT_NO1 &&
        mountPosition <= DJI_MOUNT_POSITION_PAYLOAD_PORT_NO3) {
        ...
        USER_LOG_INFO("Skip payload stream: mount position %d is not a gimbal port", mountPosition);
    }
